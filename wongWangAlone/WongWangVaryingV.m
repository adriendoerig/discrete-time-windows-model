...
forceDecision = 1;
if forceDecision % set success to 0 to return a huge error during param fitting if no decision is reached
    decision = 0.5; 
    success = 0;
else
    decision = 0.5; % set success to 1 to nt care about whether a decision is reached. if ne decision is reached, return decision = 0.5 (avg. response)
    success = 1;
end
RT = 800;
T_total = length(v);
...
for t = 1:length(r1_traj)
    if r1_traj(1,t)>=threshold
        decision = 1;
        RT = 2*t; % we want ms but there is one number in r1_traj every TWO millieconds
        success = 1;
        break
    end
